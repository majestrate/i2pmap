...
b64encode = lambda x : base64.b64encode(x, b'~-').decode('utf-8')
...
class Entry:
    ...
    def _read_string(fd):
        ...
        if slen:
            return Entry._read(fd, slen).decode('utf-8')
    ...
    def _read_addr(fd):
        ...
        if addr.valid():
            ...
            try:
                addr.location = geo_lookup(addr.options.get('host', None))
            except:
                addr.location = None
            ...
            if not addr.location:
                ...
                try:
                    addr.location = geo_lookup(addr.options.get('ihost0', None))
                except:
                    addr.location = None
    ...
    def dict(self):
        ...
        return dict({
            ...
            'options':self.options,
            'addrs':[a.dict() for a in self.addrs],
            'cert':self.cert,
